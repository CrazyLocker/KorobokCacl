...
type PageType = 'calculator' | 'cliche' | 'congrev' | 'silkscreen' | 'references' | 'settings';
                        ...
                        <Container maxWidth={false} sx={{ mt: { xs: 1.5, sm: 2, md: 3 }, px: { xs: 1, sm: 1.5, md: 2, lg: 3 }, maxWidth: { xs: '100%', sm: '100%', md: '98%', lg: '95%', xl: '1600px' }, mx: 'auto' }}>
                            <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2, md: 3, lg: 4 }, borderRadius: { xs: '8px', sm: '10px', md: '12px' }, boxShadow: '0 2px 8px rgba(0,0,0,0.12)' }}>
                                ...
                                <Box sx={{ backgroundColor: '#f8f9fa', borderRadius: { xs: '8px', sm: '10px', md: '12px' }, p: { xs: 1.5, sm: 2, md: 3 }, mb: 3, border: '1px solid #e8eaed' }}>
                                    <Typography sx={{ fontSize: { xs: '11px', sm: '12px', md: '13px' }, fontWeight: 500, color: '#5f6368', textTransform: 'uppercase', letterSpacing: '0.05em', mb: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
                                ...
                                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: { xs: 2, sm: 2.5, md: 3 }, mb: 3, minWidth: 0 }}>
                                    <Box sx={{ backgroundColor: '#f8f9fa', borderRadius: { xs: '8px', sm: '10px', md: '12px' }, p: { xs: 1.5, sm: 2, md: 2.5 }, border: '1px solid #e8eaed', overflow: 'hidden', minWidth: 0 }}>
                                                    ...
                                                    <Box key={i} sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, py: 0.75, borderBottom: '1px solid #e8eaed', gap: { xs: 0.5, sm: 0 } }}>
                                                        <Typography sx={{ fontSize: { xs: '12px', sm: '13px' }, display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'wrap', whiteSpace: 'nowrap' }}>
                                                            ...
                                                            <Box component="span" sx={{ fontSize: { xs: '9px', sm: '10px', md: '11px' }, color: '#9aa0a6', whiteSpace: 'nowrap' }}>
                                                                ({d.sheetPrice}/{d.countOnSheet}={materialCost.toFixed(2)}+{calc.workPrice}{extraCost > 0 ? `+${labels.join('+')}${extraCost.toFixed(2)}` : ''})
                                                            </Box>
                                                        </Typography>
                                    ...
                                    <Box sx={{ backgroundColor: '#f8f9fa', borderRadius: { xs: '8px', sm: '10px', md: '12px' }, p: { xs: 1.5, sm: 2, md: 2.5 }, border: '1px solid #e8eaed', overflow: 'hidden', minWidth: 0 }}>
                                        <Typography sx={{ fontSize: { xs: '13px', sm: '14px', md: '15px' }, fontWeight: 500, mb: 1.5, pb: 1, display: 'flex', alignItems: 'center', gap: 1, borderBottom: '1px solid #e8eaed' }}>
                                ...
                                <Box sx={{ backgroundColor: '#f8f9fa', borderRadius: { xs: '8px', sm: '10px', md: '12px' }, p: { xs: 1.5, sm: 2, md: 2.5 }, border: '1px solid #e8eaed', overflow: 'hidden', width: '100%' }}>
                                    <Typography sx={{ fontSize: { xs: '13px', sm: '14px', md: '15px' }, fontWeight: 500, mb: 1.5, pb: 1, display: 'flex', alignItems: 'center', gap: 1, borderBottom: '1px solid #e8eaed', flexWrap: 'wrap' }}>
                                        Цены для клиента
                                    ...
                                    {calc.result ? (
                                        <Box sx={{ '& .MuiTable-root': { fontSize: '12px', '& th': { fontSize: '10px', padding: '4px 6px' }, '& td': { fontSize: '11px', padding: '4px 6px' } } }}>
                                            <PriceTable prices={calc.result.prices} branch={calc.result.branch} basePrice={Number(calc.result.basePrice)} onUpdatePriceList={calc.updatePriceList} onCalculate={calc.calculate} loading={calc.loading} />
                                        </Box>
                                    ) : (
                ...
                return (
                    <Box sx={{ p: 4 }}>
                        <DevStub title={
                            ...
                            activePage === 'silkscreen' ? 'Расчет шелкография' :
                            'Настройки'
                        } />
                    </Box>
                );
