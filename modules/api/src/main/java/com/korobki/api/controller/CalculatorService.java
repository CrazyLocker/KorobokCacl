...
public class CalculatorService {
    ...
    public CalculationResponse calculate(CalculationRequest request) {
        ...
        List<CostCalculator.ExtraInfo> customExtras = convertExtras(request.getCustomExtras());
        ...
        BigDecimal totalCost = costCalculator.calculateTotalCost(
                details, extras, customExtras, printEnabled, printFormat, printQuantity, workPrice);
    ...
    private List<CostCalculator.DetailInfo> convertDetails(List<DetailDto> dtos) {
        List<CostCalculator.DetailInfo> result = new ArrayList<>();
        ...
        for (DetailDto dto : dtos) {
            ...
            info.setHasLak(dto.getHasLak());
            info.setHasCongrev(dto.getHasCongrev());
            info.setHasTisnenie(dto.getHasTisnenie());
            result.add(info);
        }
        return result;
    }
    ...
    private List<CostCalculator.ExtraInfo> convertExtras(List<ExtraDto> dtos) {
        List<CostCalculator.ExtraInfo> result = new ArrayList<>();
        if (dtos == null) return result;
        for (ExtraDto dto : dtos) {
